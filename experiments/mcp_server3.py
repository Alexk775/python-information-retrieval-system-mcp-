#!/usr/bin/env python3
# mcp_server.py — V5: V3 Chunking + Your "Trust High Lexiscore" Logic
...
# Import helpers inside functions to prevent adapter crash
...
SEM_TOPK = int(os.getenv("MCP_SEM_TOPK", "100")) # Widen the net
SEM_SNIPPET = int(os.getenv("MCP_SNIPPET", "600"))

# --- V5 HYBRID LOGIC ---
# Your idea: Trust scores above this, rerank the rest.
LEXICAL_GUARANTEE_SCORE = float(os.getenv("MCP_LEXICAL_GUARANTEE", "0.85")) # Very high bar

# Fallback triggers
MIN_RESULTS = int(os.getenv("MCP_MIN_RESULTS", "3"))
MIN_TOP_SCORE = float(os.getenv("MCP_MIN_TOP_SCORE", "0.35"))
MIN_SEM_BASE = float(os.getenv("MCP_SEM_BASE", "0.05"))
...
class Index:
    ...
    def search(self, query: str, top_k: int = None, filters: dict | None = None):
        """V5 Hybrid Search: Trust high lexical scores, rerank the rest."""
        # --- LAZY LOAD HELPERS ---
            ...
            rows.append(
                {
                    ...
                    "score": round(lex_norm, 3), # This is the lexical_score
                }
            )
        ...
        # --- V5 HYBRID LOGIC ---
        # 1. Separate "obvious wins" from "maybes"
        guaranteed_wins = []
        rerank_candidates = []
        for r in rows[:SEM_TOPK]: # Get Top 100
            if r["score"] >= LEXICAL_GUARANTEE_SCORE:
                r["rerank"] = "lexical_guarantee"
                guaranteed_wins.append(r)
            else:
                rerank_candidates.append(r)

        used_semantic = False
        final_rows = guaranteed_wins # Start with our "obvious wins"
        
        # 2. Rerank *only* the "maybes"
        if SEM_ENABLED and rerank_candidates:
            try:
                candidate_map = {r['uri']: r for r in rerank_candidates}
                from helpers.semantic_rerank import rerank_semantic

                sem_rows_partial = rerank_semantic(
                    query, rerank_candidates, top_k=max(top_k, KEEP_MIN), alpha=SEM_ALPHA
                )
                
                sem_rows = []
                for partial_row in sem_rows_partial:
                    uri = partial_row.get("uri")
                    if uri in candidate_map:
                        original_row = candidate_map[uri].copy()
                        original_row['semantic_score'] = partial_row.get('semantic_score', 0)
                        original_row['score'] = partial_row.get('score', original_row['score']) 
                        sem_rows.append(original_row)
                
                used_semantic = True

                # --- Run semantic filtering logic (band, rescue) *only* on the reranked results ---
                ...
                if sem_rows:
                    ...
                    # 3) Domain-aware light demotion (only on reranked items)
                ...
                if sem_rows and band_rows:
                    final_rows.extend(band_rows) # Add the filtered semantic results
                else:
                    # Fallback if semantic check returns nothing
                    final_rows.extend(rerank_candidates[:top_k])
            
            except Exception as e:
                # Catch any error during reranking
                print(f"Semantic reranking failed: {e}", file=sys.stderr)
                final_rows.extend(rerank_candidates[:top_k])
        else:
            # Semantic disabled or no lexical results
            final_rows.extend(rerank_candidates[:top_k])
        
        # 3. Combine and apply MMR
        # We must re-sort by score *before* MMR
        final_rows.sort(key=lambda r: r.get("score", 0.0), reverse=True)

        # --- final MMR diversity ---
        safe_rows = [r for r in final_rows if 'snippet' in r and 'score' in r]
        rows = mmr_rerank(safe_rows, lam=0.65, top_k=top_k)

        # normalize to 0..1 and tag rerank origin
        for r in rows:
            final_score = r.get("score", 0.0)
            r["score"] = round(float(final_score), 3)
            # Tag the rerank origin
            if "rerank" not in r:
                r["rerank"] = "semantic" if used_semantic else "lexical"
    ...
    # ---------- V-Final: "SMART CHUNKING" PARSERS ----------
    # These replace the V2 (broken) parsers
    ...
    def _parse_image(self, path: Path, base_uri: str):
        ...
        self.chunks[f"{base_uri}#image-ocr"] = text
...
# --- V-Final FIX: Adapter Crash Fix ---
# This prints to stderr, which is safe and won't break the JSON-RPC pipe.
